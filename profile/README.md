<div align="center">

# 🏴‍☠️🌌🪐 SuperNovae 🪐🌌🏴‍☠️

### We build products. We obsess over quality. We stay small on purpose.

<br />

<img src="https://raw.githubusercontent.com/supernovae-st/hub/main/assets/supernovae-ship.gif" width="100%" alt="SuperNovae Ship" />

</div>

## Our Products

<table>
<tr>
<td width="50%" valign="top">

### 🦋 Nika
**Inference as Code**

[![Live](https://img.shields.io/badge/Status-Live-10b981?style=flat-square)](https://github.com/supernovae-st/nika)
[![GitHub](https://img.shields.io/badge/GitHub-nika-181717?style=flat-square&logo=github)](https://github.com/supernovae-st/nika)
[![AGPL-3.0](https://img.shields.io/badge/License-AGPL--3.0-green?style=flat-square)](https://github.com/supernovae-st/nika/blob/main/LICENSE)

Open source AI workflow engine. One file. Any AI. Write YAML, Nika runs it — 7 providers, parallel, structured output. Single Rust binary.

[![Rust](https://img.shields.io/badge/Rust-000000?style=flat-square&logo=rust&logoColor=white)](https://rust-lang.org)
[![YAML](https://img.shields.io/badge/YAML-CB171E?style=flat-square&logo=yaml&logoColor=white)](https://yaml.org)

</td>
<td width="50%" valign="top">

### 🔲 QRCode AI
**AI QR Code Generator**

[![Live](https://img.shields.io/badge/Status-Live-10b981?style=flat-square)](https://qrcode-ai.com)
[![Website](https://img.shields.io/badge/🌐-qrcode--ai.com-6366f1?style=flat-square)](https://qrcode-ai.com)

AI-generated artistic QR codes that actually scan. In production.

[![X](https://img.shields.io/badge/@QRCODEAI-000000?style=flat-square&logo=x)](https://x.com/QRCODEAI)
[![Product Hunt](https://img.shields.io/badge/Product_Hunt-DA552F?style=flat-square&logo=producthunt&logoColor=white)](https://www.producthunt.com/products/qrcode-ai)

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🪽 NovaNet
**Knowledge Graph**

[![Active](https://img.shields.io/badge/Status-Active-10b981?style=flat-square)](https://github.com/supernovae-st/novanet)
[![GitHub](https://img.shields.io/badge/GitHub-novanet-181717?style=flat-square&logo=github)](https://github.com/supernovae-st/novanet)

Native content generation across 200+ locales. Neo4j + Rust. The brain behind our products.

[![Neo4j](https://img.shields.io/badge/Neo4j-4581C3?style=flat-square&logo=neo4j&logoColor=white)](https://neo4j.com)

</td>
<td width="50%" valign="top">

### ⚡ Jungo
**SEO & Content Platform**

[![Building](https://img.shields.io/badge/Status-Building-f59e0b?style=flat-square)](https://github.com/supernovae-st/jungo)

Multilingual SEO monitoring and programmatic content at scale. Powered by Nika + NovaNet.

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 🌈 Rain.bo
**Link-in-bio**

[![Building](https://img.shields.io/badge/Status-Building-f59e0b?style=flat-square)](https://github.com/supernovae-st/rain-bo)

Your digital identity. Beautiful micro-sites.

</td>
<td width="50%" valign="top">

### 🏝️ Candid
**Photo App**

[![Concept](https://img.shields.io/badge/Status-Concept-64748b?style=flat-square)]()

Consumer photography. Coming soon.

</td>
</tr>
</table>

---

## How We Think

We make consumer products — apps, tools, platforms — that we'd want to use ourselves.

Quality creates adoption, not marketing. We stay small because constraints make better products. We open source our infrastructure because freedom matters more than control.

If we need to build an AI kernel to ship better products, we build an AI kernel. That's Nika.

---

<div align="center">

<table>
  <tr>
    <td align="center">
      <a href="https://github.com/ThibautMelen">
        <img src="https://github.com/ThibautMelen.png" width="100" style="border-radius:50%" alt="Thibaut"/>
      </a>
      <br />
      <b>Thibaut</b>
      <br />
      <sub>Engineering</sub>
      <br />
      <a href="https://github.com/ThibautMelen"><img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github" alt="GitHub"/></a>
      <a href="https://x.com/ThibautMelen"><img src="https://img.shields.io/badge/X-000000?style=flat-square&logo=x" alt="X"/></a>
    </td>
    <td align="center">
      <a href="https://github.com/NicolasCELLA">
        <img src="https://github.com/NicolasCELLA.png" width="100" style="border-radius:50%" alt="Nicolas"/>
      </a>
      <br />
      <b>Nicolas</b>
      <br />
      <sub>Product</sub>
      <br />
      <a href="https://github.com/NicolasCELLA"><img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github" alt="GitHub"/></a>
      <a href="https://x.com/ncella_"><img src="https://img.shields.io/badge/X-000000?style=flat-square&logo=x" alt="X"/></a>
    </td>
  </tr>
</table>

<br />

Paris · [supernovae.studio](https://supernovae.studio) · [qrcode-ai.com](https://qrcode-ai.com)

</div>
